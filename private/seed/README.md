...
[private/db](../db)) and then runs `src/index.ts`, which resets the miniflare
D1 database to a known state and fills it with a workspace that reads like one
in use.

It needs the sqlite file miniflare creates, so run `vp dev` once and make a
request that touches the database before the first seed.
...
realtime room (see [private/realtime](../realtime)), so each is built as a Yjs
