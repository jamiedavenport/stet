...
- `src/security.ts`: security headers on every worker response (HSTS, `nosniff`, and for HTML: report-only CSP, framing, referrer, permissions) plus the rate limiters. Credential endpoints are throttled per IP+path (`AUTH_RATE_LIMIT`); non-content `/api/v1` requests per API key (`API_RATE_LIMIT`). Content delivery is temporarily exempt. Auth forms optionally render a Turnstile widget (`TURNSTILE_SITE_KEY` / `TURNSTILE_SECRET_KEY`).
