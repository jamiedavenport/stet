...
told to install the [plugin](/reference/codegen) and add `stet()` to
`vite.config.ts`; anywhere else — Next.js, another bundler, a CI checkout — is
told to install [`@stetcms/client`](/reference/client) and run `stet generate`,
because the plugin would do nothing there.

Without `--force` an existing config is never replaced, and detection uses the
[same candidates](/reference/configuration) the plugin resolves, so writing to
...
<TypeTable
  type={{
    ...
    '--if-key': {
      type: 'boolean',
      description: 'Succeed without generating when no API key is set.',
      default: 'false',
    },
  }}
/>
