...
- `./access`: server-only ownership checks (`requireContentType`, `requireField`, `requireEntry`), plus `liveFields`. Public keys live in `content_field_key`; content values and rich-text roots use stable field ids, so a rename changes metadata rather than rewriting content.
- `./deprecations`: deleted keys and rename aliases awaiting migration, plus `completeFieldAction`. Completing a rename removes its alias. Completing a deletion erases the field values, bodies, revision copies, tombstone, and every alias. `apps/web`'s Actions page is the caller.
- `./body`: the rich text body schema and its serialization: TipTap extensions, body-to-markdown, sanitised body-to-HTML, and markdown-to-body. The schema and HTML allowlist live together, so an extension must be supported by both public renderers before it is added.
