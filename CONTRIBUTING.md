...
The defaults in `.dev.vars.example` run the whole app locally with no external accounts, and CI copies it verbatim. It documents each value inline: notably the absent `STET_API_KEY` and `ANALYTICS_ENABLED=false`, which between them keep local traffic out of the real analytics project, the switch covering you once you do add a key (see [dogfooding/analytics](dogfooding/analytics)), Cloudflare's always-pass Turnstile test pair, and the commented-out optional keys for Resend, Anthropic, Stripe (see [internal/billing](internal/billing) for the Stripe CLI sandbox setup), and social sign-in.
