...
- `./entry`: the convention tying content entries to their body documents: `entryPage(id)`, `bodyField(fieldId)`, `bodyFragment(doc, fieldId)`, and the plain-text extraction the search mirror indexes with.
- `./version`: the counter a room carries to say its content changed. `notifyContentChanged(room)` (in `./document`) bumps it inside the Durable Object, which broadcasts it like any other edit; pages watch it with `observeContentVersion` and read their content again. Nothing reads the value. `@repo/content` calls this on every write, so a change made through the public API, the assistant's tools, or an import lands on an open table the same way an editor's keystroke does.
