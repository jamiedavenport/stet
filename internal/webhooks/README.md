...
Add the new type to `src/registry.ts` and to the inlined event enum in `internal/api/src/contract.ts` (that file stays free of workspace imports so OpenAPI generation can run it under Node). The registry carries a `satisfies` check against the contract enum, so forgetting either side is a compile error, not silent drift.
