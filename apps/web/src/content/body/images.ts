 ...
 * editor down with it.
 */
export function useBodyImages(editor: Editor, invalidMessage: string): BodyImages {
  ...
  const uploadFiles = useCallback(
    (files: File[]) => {
      const accepted = files.filter(
        (file) =>
          acceptsContentType('body-image', file.type) && acceptsSize('body-image', file.size),
      );
      ...
      void (async () => {
        try {
          for (const file of accepted) {
            const { url } = await uploadFile('body-image', file);
