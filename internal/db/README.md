...
```bash
pnpm push          # the local sqlite miniflare uses
...
`scripts/ensure-local-db.ts` runs first on the local target. Miniflare only
creates its sqlite file on the first query a request happens to make, so on a
fresh clone `drizzle-kit push` would otherwise have nothing to open.
